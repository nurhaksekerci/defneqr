...
exports.createPromoCode = async (req, res, next) => {
  try {
    ...
    if (existingCode) {
      return res.status(400).json({
        success: false,
        message: 'Bu promosyon kodu zaten mevcut'
      });
    }
...
// Tüm promosyon kodlarını listele (Admin)
...
// Promosyon kodunu doğrula ve detaylarını getir
exports.validatePromoCode = async (req, res, next) => {
  try {
    ...
    if (!promoCode.isActive) {
      return res.status(400).json({
        success: false,
        message: 'Bu promosyon kodu artık geçerli değil'
      });
    }
    ...
    if (promoCode.validFrom > now) {
      return res.status(400).json({
        success: false,
        message: 'Bu promosyon kodu henüz geçerli değil'
      });
    }
    ...
    if (promoCode.maxUses && promoCode.usedCount >= promoCode.maxUses) {
      return res.status(400).json({
        success: false,
        message: 'Bu promosyon kodu kullanım limitine ulaşmış'
      });
    }
    ...
    if (planId && promoCode.applicablePlans) {
      ...
      if (!applicablePlans.includes(planId)) {
        return res.status(400).json({
          success: false,
          message: 'Bu promosyon kodu seçtiğiniz plan için geçerli değil'
        });
      }
    }
    ...
    if (req.user) {
      ...
      if (previousUsage) {
        return res.status(400).json({
          success: false,
          message: 'Bu promosyon kodunu daha önce kullandınız'
        });
      }
    }
...
// Promosyon kodunu kullan (abonelik sırasında)
...
// Promosyon kodunu güncelle (Admin)
exports.updatePromoCode = async (req, res, next) => {
  try {
    const { id } = req.params;
    const updates = req.body;

    // Code güncelleniyorsa, benzersizlik kontrolü
    if (updates.code) {
...
// Promosyon kodu kullanım geçmişi (Admin)
...
// Kullanıcının kendi promosyon kodu kullanımları
