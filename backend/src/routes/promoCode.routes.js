...
const createPromoCodeValidation = [
  body('code')
    .trim()
    .notEmpty().withMessage('Kod gereklidir')
    .isLength({ min: 3, max: 50 }).withMessage('Kod 3-50 karakter arasında olmalıdır')
    .matches(/^[A-Z0-9_-]+$/i).withMessage('Kod sadece harf, rakam, tire ve alt çizgi içerebilir'),
  body('type')
    .isIn(['PERCENTAGE', 'FIXED', 'FREE_TRIAL']).withMessage('Geçersiz kod tipi'),
  body('discountValue')
    .isFloat({ min: 0 }).withMessage('İndirim değeri 0 veya daha büyük olmalıdır'),
  body('maxUses')
    .optional()
    .isInt({ min: 1 }).withMessage('Maksimum kullanım 1 veya daha büyük olmalıdır'),
  body('validFrom')
    .optional()
    .isISO8601().withMessage('Geçerli tarih formatı değil'),
  body('validUntil')
    .optional()
    .isISO8601().withMessage('Geçerli tarih formatı değil'),
  body('description')
